...
A drop-in instruction file for Claude Code and the Claude API. Save as `CLAUDE.md` at the root of your repository, then adapt the **Project context** and **Common commands** sections to your codebase. Keep it short — if it exceeds one page, split or prune.
...
- **Read `CONTEXT.md` first if it exists.** It is the small, always-on source of truth for goals, state, and constraints. Do not ask for what it already contains.
- **Open linked documents only when relevant.** `ARCHITECTURE.md`, decision records, and specs are referenced from `CONTEXT.md`; read them when the task touches them, not by default.
...
<!-- Replace with your project's real commands. -->

- Install: `<!-- e.g. pnpm install -->`
- Test: `<!-- e.g. pnpm test -->`
- Lint: `<!-- e.g. pnpm lint -->`
- Build: `<!-- e.g. pnpm build -->`
- Run locally: `<!-- e.g. pnpm dev -->`
