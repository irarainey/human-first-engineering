...
### What is context engineering, and is it just prompting?

No. Prompting is how you phrase a single request; context engineering is the knowledge that request draws on. Context is the durable asset — repository instructions, architectural decisions, conventions, domain constraints — captured where it outlives any one session and serves the next human as much as the next prompt. If you find yourself explaining the same background to the AI repeatedly, that background belongs in your context, not in another prompt. Treat it like code: capture it, keep it in the repository, version it, review it, own it.
...
### Is reviewing AI-generated code different from normal code review?

Yes, enough to treat it as its own skill. There is no human author in the room who reasoned their way to the change and can walk you through it, the volume is higher, and the output is often plausible on the surface while subtly wrong underneath. Review the reasoning, not just the diff; be most sceptical where the code looks most convincing; and check the edge cases and assumptions the AI never mentioned. Calibrate depth to risk — you cannot skim a large generated change the way you skim a two-line fix. Getting good at this is one of the most valuable things you can do in an AI-native team.
