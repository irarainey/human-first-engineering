...
## 🏷️ A narrower term, in wider use

"Context engineering" is a young phrase. It traces to a [June 2025 tweet from Shopify's Tobi Lütke](https://blog.getzep.com/what-is-context-engineering/) — "the art of providing all the context for the task to be plausibly solvable by the LLM" — sharpened days later by Andrej Karpathy into "filling the context window with just the right information for the next step." Mainstream usage since has largely kept that scope: assembling what a *model* needs — a system prompt, retrieved facts, tool outputs, conversation and user history — so it performs the task reliably. [Anthropic's own definition](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents) makes the boundary explicit: "curating and maintaining the optimal set of tokens (information) during LLM inference." Some of that usage (agent memory, temporal user profiles) already spans many sessions; the boundary that matters is not how long the context lives, but what it holds and who it is for.

[Levels of Agentic Engineering](https://www.bassimeledath.com/blog/levels-of-agentic-engineering) is one example, placing context engineering as level 3 of 8: managing token density and information relevance within a session's working context, a practice teams move past once models tolerate noisier input. Others extend it further — persistent memory of user facts and preferences fed back to the model at runtime — but the content is still model-facing: information *about* a task or a user, assembled to make the next model call succeed.

Human-First Engineering uses the same words for something with a different subject and audience: not facts fed to a model, but a team's engineering reasoning — decisions, rejected alternatives, architecture, constraints — written for the humans on the team first, and read by AI as a side effect. The table above already draws that line for a single interaction; this page is about the durable, team-owned version. Where this page says "context engineering," read that wider, team-owned practice, not the model-facing one described above.

None of the underlying discipline is new — notes, runbooks, wikis, and ADRs are how teams have always kept reasoning from evaporating, long before any of it had an AI attached. What AI adds is the constraint from the opening section: a context window that is finite and paid for in tokens on every read, so a person can skim a wiki and quietly discard what does not matter, but an assistant fed everything at once cannot. One bloated instructions file destroys the fidelity that discipline was meant to protect. The real engineering problem is not inventing documentation practice — it is making it survive a small, expensive window, which is why this page insists on a small always-on core and a linked file opened only when the task needs it.
